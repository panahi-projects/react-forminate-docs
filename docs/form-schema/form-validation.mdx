...
sidebar_position: 3
---
import { Code3, Code4, Code5, Code6, Code7, Code8, Code9, Code10, Code11, Code12, Code13, Code14 } from "@site/src/components/Codes"
...
## Error Handling
### Accessing Errors
Errors are available through the `useFormErrors` hook:
...
By wrapping the app by using of the provider `<FormRegistryProvider>{children}</FormRegistryProvider>` you can access to the various numbers of custom hooks that the **React-Forminate** provides you, and by using of the `useFormErrors()` you can get the form's validation errors.

You can see the usage of the provider and the relevant hook in the below example:

<Code12 
      description="Demonstrates custom error handling in React-Forminate using FormRegistryProvider and useFormErrors hook, with manual error display for form validation messages."
      features={[
        "Custom error display",
        "FormRegistryProvider usage",
        "useFormErrors hook",
      ...
      keywords={[
        ...
        "React-Forminate error handling",
        "form error customization"
      ]}
/>
...
Key Features Demonstrated:
    ...
    1. Manual Validation Trigger - Validate the entire form programmatically
    2. Form State Access - Retrieve current form values and validation status
    3. Custom Submission Handling - Implement your own submission logic

Implementation Example:

<Code13 
      description="Demonstrates manual form validation triggering in React-Forminate using useFormActions and useFormValues hooks, with custom submit button and validation state handling."
      features={[
      ...
      keywords={[
        "React manual form validation",
        "trigger validation programmatically",
        "useFormActions hook",
        "useFormValues hook",
        "custom form submission",
        "React-Forminate validation control",
...
The example shows how to combine `useFormValues` and `useFormActions` to create a flexible validation flow while maintaining access to all form state.

## Complete Example

<Code14 
      description="Complete user registration form example with email validation, password field, age verification, and newsletter subscription using React-Forminate's validation system."
      features={[
        "Email format validation",
